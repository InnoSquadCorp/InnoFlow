...
`main` は 5.0 の開発契約を説明し、下記のインストール例は最新の安定版 4.0.0 を維持します。
...
- `TestStore.exhaustivity` のデフォルトは `.on` で、すべての状態遷移と effect action を漏れなく検証します。テストは `finish()` で終了し、未検証の作業を残した deinit はポリシーに従って失敗、警告、または無通知で処理されます。`EffectTask.run` から漏れた cancellation 以外のエラーは、このポリシーに関係なく元の action assertion 位置で一度失敗します。
