...
non-UI feature/domain target は `InnoFlow` だけに依存できます。SwiftUI app target は
`InnoFlowSwiftUI` も依存に追加して、`Store.binding`, `ScopedStore.binding`,
`Store.preview`, `EffectTask.animation(Animation?)` を使います。
