...
comes from one to three explicit state slices, prefer `select(dependingOn:..., transform:)`; plain
`select { ... }` remains the always-refresh fallback because general closures do not expose their
dependencies.
