...
- `store.binding(_:to:)` — argument-label alias for `store.binding(_:send:)` that reads naturally when passing an enum case constructor, as in `store.binding(\.$step, to: Feature.Action.setStep)`. The two spellings are semantically identical; `send:` stays supported indefinitely. Alias is defined on both `Store` and `ScopedStore`.
...
- Sample app, DocC walkthrough, `.cursor` authoring rules, README, and `CLAUDE.md` now use the new `store.binding(_:to:)` alias when forwarding an enum case constructor. `store.binding(\.$step, to: Feature.Action.setStep)` is the recommended form — the underlying behaviour is identical to the previous `send:` spelling, and `send:` remains supported indefinitely (the test suite keeps six closure-form call sites on `send:` so both code paths stay exercised).
