...
`main` 描述 5.0 开发契约；下面的安装示例仍指向最新稳定版本 4.0.0。
...
- `TestStore.exhaustivity` 默认为 `.on`，会完整验证所有状态转换和 effect action。测试应以 `finish()` 结束；若 deinit 时仍有未验证工作，则会按策略记录失败、警告或保持静默。`EffectTask.run` 中除取消以外的未处理错误不受该策略影响，并会在原始 action assertion 位置记录一次失败。
